...
resource_validation_failures[vaildation_failure] {
  ...
  vaildation_failure := get_validation_failure_for_resource(nsg_rules_with_protocol_type_any[_], "Network security group rule must not use 'Any' type, must select TCP/UDP/ICMP.", "change_nsg_rule_protocol_type.yaml", {"id": nsg_rules_with_protocol_type_any[_].id, "new_protocol": "FIXME: Tcp/Udp/Icmp"})
}
